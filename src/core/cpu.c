...
static void sn_OpBIT_dp(emGeneral* emulator);
static void sn_OpBIT_addr(emGeneral* emulator);
static void sn_OpBIT_dpX(emGeneral* emulator);
static void sn_OpBIT_addrX(emGeneral* emulator);

static void sn_OpBIT_const(emGeneral* emulator) {
	hAddr = (emulator->cpu->A & ++*emulator->cpu->PC);
	emulator->cpu->sn_ZFlag = (hAddr == 0 ? 1 : 0);
};
